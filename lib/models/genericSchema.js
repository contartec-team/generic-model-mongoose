...
const MongooseEventsEnum = require('../enums/MongooseEventsEnum')
 ...
 * @param {(Function | Boolean)} [dbConnectFunction] The `db` connect `function` or `true` if its a `Lambda` function (for `Lambda` and alikes, see {@link https://mongoosejs.com/docs/lambda.html})
 *
...
function genericSchema(schema, options, dbConnectFunction) {
  ...
  if (dbConnectFunction) {
    const connectionFunction = dbConnectFunction instanceof Function ?
      dbConnectFunction :
      mongooseConnection
